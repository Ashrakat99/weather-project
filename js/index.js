...
const dayAfterTomorrowConditionImg = document.querySelector(
    ".dayAfterTomorrow img"
);
const dayAfterTomorrowTemp = document.querySelector(".dayAfterTomorrow .temp");
const dayAfterTomorrowTempMini = document.querySelector(
    ".dayAfterTomorrow .tempSmall"
);
const dayAfterTomorrowCondition = document.querySelector(
    ".dayAfterTomorrow .condition"
);
...
try {
    const request = await fetch(
        baseUrl +
        `/forecast.json?key=b6b8827d6b644e0d99c213457242806&q=${city}&days=3&wind100mph=yes&wind100kph=yes&aqi=yes`
    );
...
function showWeatherData(obj) {
    ...
    tomorrowConditionImg.setAttribute(
        "src",
        `https:${obj.forecast.forecastday[1].day.condition.icon}`
    );
    ...
    dayAfterTomorrowConditionImg.setAttribute(
        "src",
        `https:${obj.forecast.forecastday[2].day.condition.icon}`
    );
...
try {
    ...
    alertNameCountry.innerHTML =
        "Sorry, something went wrong so can't find your location.";
    alertNameCountry.classList.replace("d-none", "d-block");
}
});
